...
const GameComponentBot = () => {
    ...
    useEffect(() => {
        ...
        const stompClient = new Client({
            ...
            onConnect: () => {
                ...
                stompClient.subscribe('/topic/duel/update', (message) => {
                ...
                stompClient.subscribe(`/topic/duel/start`, (message) => {
                    const gameUpdate = JSON.parse(message.body.toString());
                    handleGameStart(gameUpdate);
                });
                stompClient.subscribe(`/topic/duel/endGame`, (message) => {
    ...
    const handleGameUpdate = (gameUpdate) => {
        ...
        setUserField(gameUpdate.userField);
        setEnemyField(gameUpdate.enemyField);
        const updatedUserHp = gameUpdate.userId === userId ? gameUpdate.userHp : gameUpdate.enemyHp;
        const updatedEnemyHp = gameUpdate.userId === userId ? gameUpdate.enemyHp : gameUpdate.userHp;
        setUserHp(updatedUserHp);
        setEnemyHp(updatedEnemyHp);
        setCurrentTurnUserId(gameUpdate.currentTurnUserId);

        if (updatedUserHp <= 0) {
            endGame('user');
        } else if (updatedEnemyHp <= 0) {
            endGame('enemy');
        }
    };
...
export default GameComponentBot;
